  ...
  async resolveTaskJob(sessionID: string): Promise<void> {
    ...
    try {
      ...
      if (textMessages.length === 0) {
        this.logger.warn(`no text messages found session=${sid}`);
      ...
      this.logger.error(`resolveTaskJob failed session=${this.hctx.shortId(sessionID)}:`, error);
